...
if(isset($_POST['add'])){
    if(!empty($id)&& !empty($name)&& !empty($age)&&!empty($study)&&!empty($hospital)&&!empty($exp)&&!empty($expart)&&!empty($p_address)&&!empty($message)){
        mysql_query("INSERT INTO home_doctor(`id`,`name`,`age`,`study`,`hospital`,`exp`,`expart`,`p_address`,`message`)VALUES('$id','$name','$age','$study','$hospital','$exp','$expart','$p_address','$message')");
        if(mysql_affected_rows()>0){
            $img="../image/home_doctor/$id.jpg";
...
if(isset($_POST['delete'])){
    mysql_query("DELETE FROM home_doctor WHERE id='$id'");
    if(mysql_affected_rows()>0){
...
if(isset($_POST['edit'])){
    mysql_query("UPDATE home_doctor SET name='$name',age='$age',study='$study' WHERE id='$id'");
    if(mysql_affected_rows()>0){
        $su='Edit Successfull';
    }else{
       ...
       <div class="row">
           <div class="col-md-6 col-md-offset-1">
               <table class="table table-bordered">
                   ...
                   <?php
                    $select_table=mysql_query("SELECT * FROM home_doctor");
